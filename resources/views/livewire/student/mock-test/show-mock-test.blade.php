                ...
                <div class="flex items-center gap-6">
                    <div class="flex items-center gap-4">
                        ...
                        <div class="text-gray-600" wire:poll.1s="decrementTime">
                            ...
                            <span class="ml-2 text-red-600 font-bold">{{ gmdate("H:i:s", $timeLeft) }}</span>
                        </div>
            ...
            <div class="lg:col-span-3">
                ...
                <div class="bg-white border rounded-xl p-6">
                    ...
                    <div wire:key="question-{{ $questions[$currentQuestionIndex]['id'] }}" class="space-y-3 mb-6">
                        ...
                        <label
                            class="block w-full p-4 border rounded-xl hover:bg-blue-50 hover:border-blue-200
                    ...
                    <div class="flex justify-between items-center">
                        <div class="space-x-3">
                            <button wire:click="previousQuestion"
                                class="inline-flex items-center px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white font-semibold rounded-lg shadow-md transition duration-300
                                            {{ $submitted || $currentQuestionIndex === 0 ? 'cursor-not-allowed opacity-60' : '' }}" @if($submitted ||
                                $currentQuestionIndex===0) disabled @endif>
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            @if($currentQuestionIndex < count($questions) - 1) <button wire:click="nextQuestion"
                                class="inline-flex items-center px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white font-semibold rounded-lg shadow-md transition duration-300
                                            {{ $submitted || $currentQuestionIndex === count($questions) - 1 ? 'cursor-not-allowed opacity-60' : '' }}" @if($submitted ||
                                $currentQuestionIndex===count($questions) - 1) disabled @endif>
                                Next
                        ...
                        <button wire:click="submitTest" class="inline-flex items-center px-6 py-2 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-lg shadow-md transition duration-300
                                        {{ $submitted ? 'cursor-not-allowed opacity-60' : '' }}" @if($submitted)
                            disabled @endif>
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"

<div>
    <div class="py-6 px-4 md:px-8 max-w-7xl mx-auto">
        <!-- Header Section -->
        <div class="mb-8">
            <h1 class="text-2xl md:text-3xl font-bold text-gray-800">Mock Tests</h1>
            <p class="mt-2 text-gray-600">Practice and improve your skills with our mock tests</p>
        </div>
        ...
        <div class="mb-8">
            <div class="flex items-center justify-center space-x-4 md:space-x-8">
                <div class="flex items-center">
                    <span class="w-8 h-8 rounded-full flex items-center justify-center {{ $selectedCourseId ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-600' }}">1</span>
                    <span class="ml-2 text-sm font-medium {{ $selectedCourseId ? 'text-blue-600' : 'text-gray-600' }}">Select Course</span>
                </div>
                <div class="flex-1 h-0.5 {{ $selectedCourseId ? 'bg-blue-600' : 'bg-gray-200' }}"></div>
                <div class="flex items-center">
                    <span class="w-8 h-8 rounded-full flex items-center justify-center {{ $selectedLevel ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-600' }}">2</span>
                    <span class="ml-2 text-sm font-medium {{ $selectedLevel ? 'text-blue-600' : 'text-gray-600' }}">Choose Level</span>
                </div>
                <div class="flex-1 h-0.5 {{ $selectedLevel ? 'bg-blue-600' : 'bg-gray-200' }}"></div>
                <div class="flex items-center">
                    <span class="w-8 h-8 rounded-full flex items-center justify-center {{ ($selectedCourseId && $selectedLevel) ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-600' }}">3</span>
        ...
        <div class="flex justify-end mb-6">
            <button wire:click="resetSelection" class="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
            ...
            <div class="animate-fade-in">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div wire:click="selectCourse({{ $course->id }})"
                        ...
                        <div class="p-5">
                            <h3 class="text-lg font-medium text-gray-900 group-hover:text-blue-600">{{ $course->title }}</h3>
                            <p class="mt-2 text-sm text-gray-600">Click to select this course and view available tests</p>

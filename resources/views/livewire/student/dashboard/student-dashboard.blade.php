                ...
                <div class="p-6 md:p-8">
                    ...
                    <div class="text-center">
                        <a wire:navigate href="{{ route('v2.student.mycourses') }}">
                            <button class="px-6 py-3 bg-orange-500 text-white font-semibold rounded-lg hover:bg-orange-600 focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 transition duration-200">
                                Explore Courses
                            </button>
                        </a>
                    </div>
                ...
                <div class="lg:col-span-2 bg-white rounded-lg shadow-sm overflow-hidden">
                    <div class="border-b border-gray-200">
                        <div class="px-6 py-4 flex items-center justify-between">
                            ...
                            <a wire:navigate href="{{ route('v2.student.mycourses') }}" class="text-sm text-purple-700 hover:text-purple-800 font-medium">View All</a>
                        </div>
                    </div>
                    ...
                    <div class="p-6">
                        <ul class="space-y-4">
                            @foreach ($payments as $payment)
                                <li class="bg-gray-50 p-4 rounded-lg flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border border-gray-100 hover:border-purple-200 transition duration-200">
                                    ...
                                    <div class="flex flex-col items-end w-full sm:w-auto mt-3 sm:mt-0">
                                        <div class="w-full sm:w-40 bg-gray-200 rounded-full overflow-hidden h-2.5">
                                            <div class="bg-purple-600 h-2.5 rounded-full" style="width: {{ $payment->progress }}%;"></div>
                                        </div>
                        ...
                        <ul class="space-y-3">
                            @if ($assignments->isNotEmpty())
                                @foreach ($assignments as $assignment)
                                    <li class="p-3 rounded-lg border border-gray-100 hover:border-purple-200 transition duration-200">
                                                ...
                                                @if ($assignment->uploads->isNotEmpty())
                                                    @foreach ($assignment->uploads as $upload)
                                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                                            @if ($upload->status == 'submitted') bg-green-100 text-green-800
                                                            @elseif($upload->status == 'graded') bg-blue-100 text-blue-800
                                        ...
                                        <a wire:navigate href="{{ route('v2.student.message.view', $message) }}" 
                                           class="flex items-center p-3 rounded-lg border border-gray-100 hover:border-purple-200 transition duration-200">
                                            <div class="flex-shrink-0 mr-3">
                                                <span class="{{ $message->type == 'info' ? 'text-blue-500' : ($message->type == 'warning' ? 'text-yellow-500' : 'text-red-500') }}">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                                                    </svg>
                                                </span>

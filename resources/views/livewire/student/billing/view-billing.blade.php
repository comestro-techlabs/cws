<div x-data="{ activeTab: 'invoice' }" class="container mx-auto px-4 py-4">
    <!-- Tabs -->
    <div class="mb-4">
        <ul class="flex border-b">
            <li class="mr-1">
                <a @click.prevent="activeTab = 'invoice'" :class="{ 'border-blue-500 text-blue-500': activeTab === 'invoice' }" class="bg-white inline-block py-2 px-4 text-blue-500 hover:text-blue-800 font-semibold" href="#">Invoice</a>
            </li>
            <li class="mr-1">
                <a @click.prevent="activeTab = 'membership'" :class="{ 'border-blue-500 text-blue-500': activeTab === 'membership' }" class="bg-white inline-block py-2 px-4 text-blue-500 hover:text-blue-800 font-semibold" href="#">Membership</a>
            </li>
        </ul>
    </div>
    ...
    <div>
        <!-- Invoice Tab -->
        <div x-show="activeTab === 'invoice'" class="p-4 bg-white rounded-lg shadow-md">
            <table class="min-w-full bg-white divide-y divide-gray-200">
                <thead class="bg-gray-100">
                    <tr>
                        <th class="p-2 text-center text-xs font-medium text-gray-600">Course / Workshop Name</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="p-2 text-center text-xs text-gray-800">{{ $item->order_id }}</td>
                        <td class="p-2 text-center text-xs text-gray-800">
                            ...
                            <div class="flex items-center justify-center rounded-full bg-red-500 uppercase px-2 py-1 text-center text-xs font-bold mr-3">
                                {{$item->status}}
                            </div>
                            @else
                        ...
                        <td class="p-2 text-gray-800 text-center text-xs">{{ $item->method ?? 'N/A'}}</td>
                        <td class="p-2 text-gray-800 text-center text-xs">₹{{ $item->transaction_fee }}</td>
                        <td class="p-2 text-gray-800 text-center text-xs">{{ \Carbon\Carbon::parse($item->transaction_date)->format('d M Y') }}</td>
                        <td class="p-2 text-gray-800 text-center text-xs">{{ \Carbon\Carbon::create()->month((int)$item->month)->format('M') }}</td>
                        <td class="p-2 text-gray-800 text-center text-xs">{{ $item->year}}</td>
                        <td class="p-2 text-gray-800 text-left truncate max-w-xs text-xs" title="{{ $item->error_reason }}">{{ $item->error_reason }}</td>
                        <td class="p-2 text-gray-800 text-center text-xs">
                            ...
                            <button class="pay-now-button py-2.5 px-6 text-xs bg-green-500 text-white rounded-lg font-semibold shadow-xs transition-all duration-500 hover:bg-green-700"
                            ...
                            @else
                            <button class="refresh-payment py-2.5 px-6 text-xs bg-indigo-900 text-white rounded-lg font-semibold shadow-xs transition-all duration-500 hover:bg-indigo-700" data-order-id="{{ $item->order_id }}">Refresh</button>
                            @endif
                        </td>
                    </tr>
                    @endif
        ...
        <!-- Membership Tab -->
        <div x-show="activeTab === 'membership'" class="p-4 bg-white rounded-lg shadow-md">
            <table class="min-w-full bg-white divide-y divide-gray-200">
                <thead class="bg-gray-100">
                    <tr>
                ...
                <tbody class="divide-y divide-gray-200 text-xs">
                    ...
                    <tr>
                        <td class="px-2 py-1 text-gray-800 text-center">{{ \Carbon\Carbon::parse($item->due_date)->format('d M Y') }}</td>
                        <td class="px-2 py-1 text-gray-800 text-center">{{ \Carbon\Carbon::create((int)$item->year, (int)$item->month, 1)->format('M Y') }}</td>
                        <td class="px-2 py-1 text-center text-gray-800">{{ $item->order_id }}</td>

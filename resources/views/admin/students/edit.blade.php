    ...
    <div class="flex px-[2%] py-0 flex-col">
        ...
        <div class="flex flex-1 md:flex-row flex-col md:items-start gap-5">
            ...
            {{-- <div class="flex flex-[1.3] gap-3 flex-col">
                <div class="flex flex-1 flex-col bg-slate-100 p-4 rounded-lg border border-slate-300 shadow-sm">
                    ...
                    <div class="mb-5 bg-white p-4 rounded-lg shadow-sm">
                        <form action="{{ route('students.assignCourse', ['student' => $student->id]) }}" method="POST"
                            class="flex flex-col gap-4">
                            @csrf
                            <div>
                                <label for="course_id" class="block text-sm font-medium text-slate-600">Select
                                    Course</label>
                                <select name="course_id" id="course_id"
                                    class="mt-1 px-3 py-2 block w-full border border-slate-300 rounded-md shadow-sm focus:ring-teal-500 focus:border-teal-500 sm:text-sm">
                            ...
                            <button type="submit"
                                class="bg-teal-600 text-white px-4 py-2 rounded-md hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-teal-500">Assign
                                Course</button>
                        </form>
                    </div>
                    ...
                    <div>
                        ...
                        <ul class=" space-y-2">
                            @foreach ($student->courses as $course)
                                <li
                                    class="flex justify-between items-center bg-white p-2 rounded-md border border-slate-200 shadow-sm gap-2">
                                    <span class="text-slate-800 text-sm capitalize">{{ $course->title }}</span>
                                    <form
                                        action="{{ route('students.removeCourse', ['student' => $student->id, 'course' => $course->id]) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                    ...
                                    <li class="flex flex-col bg-slate-50 p-3 rounded-lg shadow-sm">
                                        ...
                                        <div class="flex justify-between mb-2 text-slate-600">
                                            @if ($payment->status == 'due' && $payment->due_date)
                                                <span class="text-sm">Due Date:
                                                    {{ $payment->due_date->format('d M, Y') }}</span>
                                            @endif
                                        </div>
                                        <div class="flex items-center justify-end">
                                            @if ($payment->status == 'due')
                                                <form
                                                    action="{{ route('students.processPayment', ['student' => $student->id]) }}"
                                                    method="POST">
                                                    @csrf
                                                    ...
                                                    <input type="hidden" name="payment_option"
                                                        value="{{ $paymentOption }}">
                                                    <button type="submit"
                                                        class="bg-teal-600 text-white text-xs px-3 py-1 rounded hover:bg-teal-700 transition">Pay
                                                        Now</button>
                                                </form>
                                            @else
            ...
            <div>
                ...
                <ul>
                    @foreach ($purchasedCourses as $payment)
                        <li>
                            Course: {{ $payment->course->title }} <br>
                            Description: {{ $payment->course->description }} <br>
                            Fees: {{ $payment->course->fees }} <br>
                            Payment Amount: {{ $payment->amount }} <br>
                            Payment Date: {{ $payment->payment_date }} <br>
                            Status: {{ $payment->payment_status }} <br>
            ...
            <div>
                <h2>Payments Grouped by Course</h2>
                @foreach ($paymentsGroupedByCourse as $courseId => $payments)

 ...
 --}}
  ...
  <div {{-- data-popover-target="popover-course-details{{$course->id}}" data-popover-placement="right" --}}
      class="flex border border-slate-300 rounded-t-xl rounded-b-xl w-full max-w-xs flex-col bg-white text-gray-700">
      {{-- <a href="{{route('public.courseDetails',['category_slug' => $course->category->cat_slug, 'slug' =>  $course->slug])}}" class="flex flex-1 flex-col"> --}}
      <a
          href="{{ route('public.courseDetails', ['category_slug' => optional($course->category)->cat_slug, 'slug' => $course->slug]) }}"class="flex flex-1 flex-col">

          <div class="overflow-hidden bg-blue-gray-500 shadow-lg min-h-[310px] text-center ">
              <img src="storage/course_images/{{ $course->course_image }}" alt="{{ $course->title }}"
                  class="w-full  rounded-t-xl object-cover h-auto" />
          </div>
          <div class="p-3">
              <div class="flex items-center justify-between">
                  <h5 class="text-lg capitalize font-bold text-black truncate">
